...
while(1)
{
     ...
     for(i=0; i<bytesPerSector; i+=32)
     {
        ...
        if(dir->name[0] == EMPTY) //indicates end of the file list of the directory
		{
		  if((flag == GET_FILE) || (flag == DELETE))
			  uart_send_string_from_FLASH(PSTR("File does not exist!"));
		  return 0;
		}
		if((dir->name[0] != DELETED) && (dir->attrib != ATTR_LONG_NAME))
        {
          if((flag == GET_FILE) || (flag == DELETE))
          {
            ...
            if(j == 11)
			{
			  ...
			  {
			     TX_NEWLINE;
			     uart_send_string_from_FLASH(PSTR("Deleting.."));
				 TX_NEWLINE;
				 TX_NEWLINE;
			     ...
			     while(1)
			     {
					...
					if(nextCluster > 0x0ffffff6)
					   {uart_send_string_from_FLASH(PSTR("File deleted!"));return 0;}
          ...
          else  //when flag = GET_LIST
		  {
		     ...
		     {
			   if(j == 8) uart_send(' ');
			   uart_send(dir->name[j]);
			 }
			 uart_send_string_from_FLASH(PSTR("   "));
		     if((dir->attrib != 0x10) && (dir->attrib != 0x08))
			 {
		    	 uart_send_string_from_FLASH(PSTR("FILE" ));
		    	 uart_send_string_from_FLASH(PSTR("   "));
			     displayMemory (dir->fileSize);
			 }
			 else
				 uart_send_string_from_FLASH((dir->attrib == 0x10)? PSTR("DIR") : PSTR("ROOT"));
		  }
       }
     }
   }
   ...
   if(cluster > 0x0ffffff6)
   	 return 0;
   if(cluster == 0)
   {uart_send_string_from_FLASH(PSTR("Error in getting cluster"));  return 0;}
 }
return 0;
}
...
while(1)
{
  ...
  cluster = getSetNextCluster (cluster, GET, 0);
  if(cluster == 0) {uart_send_string_from_FLASH(PSTR("Error in getting cluster")); return 0;}
}
return 0;
}
...
if(j>8) {printf("convertFileName :%s:\n",fileName); uart_send_string_from_FLASH(PSTR("Invalid fileName..")); return 1;}
...
if(j == 1)
{
  uart_send_string_from_FLASH(PSTR("  File already existing, appending data.."));
  appendFile = 1;
...
while(1)
{
   ...
   if(cluster == 0) {uart_send_string_from_FLASH(PSTR("Error in getting cluster")); return;}
...
uart_send_string_from_FLASH(PSTR("Total Memory: "));
...
TX_NEWLINE;
uart_send_string_from_FLASH(PSTR(" Free Memory: "));
